...
public abstract class MPGuiScrollPanel<T extends MPGuiScrollPanel<T>> implements MPGuiElement<T> {
    ...
    public MPGuiScrollPanel(MPGuiShape elementShape) {
    ...
    @Override
    public boolean onMouseDragged0(Minecraft mc, int mouseX, int mouseY, MPMoveDirection direction, int diffX, int diffY) {
        if (!calculatedElementShape.contains(mouseX, mouseY)) return false;
        if (content != null && content.mouseHover(mc, mouseX, mouseY)) {
    ...
    public void measurePreferred(IGuiVector pDefSize, IGuiVector pContentSize, float sugX, float sugY, MutableGuiVector result) {
        MPGuiRenderHelper.measurePreferredWithScaleRules(pDefSize, pContentSize, sugX, sugY, result, elementShape, scaleRules);
